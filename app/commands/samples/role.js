...
export async function execute(interaction) {
 ...
 const role = await guild.roles.create({ name, color, mentionable, hoist });
 logEmbed.addFields({ name: '作成したロール', value: role.name, inline: true });
 ...
 await role.delete();
 logEmbed.addFields({ name: '削除したロール', value: role.name, inline: true });
 ...
 await role.edit({ name, color, mentionable, hoist });
 logEmbed.addFields({ name: '編集したロール', value: role.name, inline: true });
 ...
 case 'add': {
  ...
  logEmbed.addFields(
    ...
    { name: '追加したロール', value: role.name, inline: true }
  );
 ...
 logEmbed.addFields(
 { name: '対象', value: target.tag, inline: true },
 { name: '削除したロール', value: role.name, inline: true }
 );
