...
export async function execute(interaction) {
 ...
 logEmbed.addFields({ name: '作成したイベント', value: name, inline: true });
 ...
 await event.delete();
 logEmbed.addFields({ name: '削除したイベント', value: eventId, inline: true });
 ...
 const endTime = interaction.options.getString('end') ? new Date(interaction.options.getString('end')) : null;
 const event = await guild.scheduledEvents.fetch(eventId);
 await event.edit({ name, description, scheduledStartTime: startTime, scheduledEndTime: endTime });
 logEmbed.addFields({ name: '編集したイベント', value: eventId, inline: true });
 ...
 logEmbed.addFields({ name: '作成した絵文字', value: name, inline: true });
 ...
 await emoji.delete();
 logEmbed.addFields({ name: '削除した絵文字', value: emojiId, inline: true });
 ...
 eventType: 1,
 triggerType: 1,
 triggerMetadata: { regexPatterns: regex ? [regex] : [], keywordFilter: keywords },
 actions: [{ type: 1 }]
 });
 logEmbed.addFields({ name: '作成したAutoModルール', value: name, inline: true });
 ...
 await rule.delete();
 logEmbed.addFields({ name: '削除したAutoModルール', value: ruleId, inline: true });
 ...
 await invite.delete();
 logEmbed.addFields({ name: '削除した招待', value: inviteCode, inline: true });
 ...
 const webhook = await interaction.channel.createWebhook({ name, avatar });
 logEmbed.addFields({ name: '作成したWebhook', value: name, inline: true });
 ...
 logEmbed.addFields({ name: '削除したWebhook', value: webhookId, inline: true });
