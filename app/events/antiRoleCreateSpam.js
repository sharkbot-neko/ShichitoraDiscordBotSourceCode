...
export async function handleRoleCreateSpam(client) {
  client.on(Events.RoleCreate, async (role) => {
    ...
    settings.antiRoleCreateSpam = settings.antiRoleCreateSpam ?? { enabled: false, nukeLimit: 10, timeframe: 60000 };
    ...
    settings.ruleWhitelist.role_create = settings.ruleWhitelist.role_create ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    const isRuleWhitelisted =
      member.roles.cache.some((role) => settings.ruleWhitelist.role_create.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.role_create.members.some((m) => m === member.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiRoleCreateSpam?.enabled) return;

    const rateLimit = settings.antiRoleCreateSpam.nukeLimit || 10;
    const timeframe = settings.antiRoleCreateSpam.timeframe || 60000;
    const points = settings.points.role_create || 1;
    const now = Date.now();
    ...
    if (!actionHistory.has(member.id)) {
      ...
      if (recentActions.length > rateLimit) {
        ...
        for (const action of recentActions) {
          const roleToDelete = await role.guild.roles.fetch(action.targetId).catch(() => null);
        ...
        if (settings.block.enabled && punishment) {
          try {
            if (punishment === 'timeout') {
              await member.timeout(settings.block.timeout || 600000, 'ロール作成制限超過');
            } else if (punishment === 'kick') {
              await member.kick('ロール作成制限超過');
            } else if (punishment === 'ban') {
              await role.guild.members.ban(member.id, { reason: 'ロール作成制限超過' });
            }
          } catch (err) {
        ...
        if (webhookUrl) {
          const embed = new EmbedBuilder()
            .setTitle(`ロール作成制限違反 from ${role.guild.name}`)
            .setDescription(
