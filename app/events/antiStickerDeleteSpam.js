...
export async function antiStickerDeleteSpam(client) {
  client.on(Events.GuildStickerDelete, async (sticker) => {
    const auditLogs = await sticker.guild.fetchAuditLogs({ type: AuditLogEvent.StickerDelete, limit: 1 }).catch(() => null);
    ...
    settings.antiStickerDeleteSpam = settings.antiStickerDeleteSpam ?? { enabled: false, nukeLimit: 5, timeframe: 60000 };
    ...
    settings.ruleWhitelist.sticker_delete = settings.ruleWhitelist.sticker_delete ?? {
      channels: [], categories: [], roles: [], members: []
    };

    const isRuleWhitelisted =
      member.roles.cache.some((role) => settings.ruleWhitelist.sticker_delete.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.sticker_delete.members.some((m) => m === member.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiStickerDeleteSpam?.enabled) return;

    const rateLimit = settings.antiStickerDeleteSpam.nukeLimit || 5;
    const timeframe = settings.antiStickerDeleteSpam.timeframe || 60000;
    const points = settings.points.sticker_delete || 4;
    const now = Date.now();
    ...
    if (!actionHistory.has(member.id)) {
      ...
      if (recentActions.length > rateLimit) {
        ...
        if (settings.block?.enabled && punishment) {
          try {
            if (punishment === 'timeout') await member.timeout(settings.block.timeout || 600000, 'ステッカー削除制限超過');
            else if (punishment === 'kick') await member.kick('ステッカー削除制限超過');
            else if (punishment === 'ban') await sticker.guild.members.ban(member.id, { reason: 'ステッカー削除制限超過' });
          } catch (err) {
        ...
        if (webhookUrl) {
          const embed = new EmbedBuilder()
            .setTitle(`ステッカー削除制限違反 from ${sticker.guild.name}`)
