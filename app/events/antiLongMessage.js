...
export async function handleLongMessage(client) {
  client.on(Events.MessageCreate, async (message) => {
    ...
    settings.antiLongMessage = settings.antiLongMessage ?? { enabled: false, lengthLimit: 500 };
    ...
    settings.ruleWhitelist.long_message = settings.ruleWhitelist.long_message ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    const isRuleWhitelisted =
      settings.ruleWhitelist.long_message.channels.some((ch) => ch === message.channelId) ||
      settings.ruleWhitelist.long_message.categories.some((cat) => cat === channel.parentId) ||
      message.member?.roles.cache.some((role) => settings.ruleWhitelist.long_message.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.long_message.members.some((m) => m === message.author.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiLongMessage?.enabled) return;

    const contentLength = message.content.length;

    if (contentLength > (settings.antiLongMessage.lengthLimit || 500)) {
      await handleViolation(message, 'long_message', settings, contentLength);
    }
  });
}

async function handleViolation(message, rule, settings, contentLength) {
  ...
  pointsData[guildId][message.author.id] = pointsData[guildId][message.author.id] || { points: 0, lastViolation: null };
  ...
  if (settings.logWebhook) {
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        `**処罰**: ${punishment || 'なし'}\n` +
        `**文字数**: ${contentLength}\n` +
        `**制限**: ${settings.antiLongMessage.lengthLimit || 500}\n` +
  ...
  if (webhookUrl) {
    const embed = new EmbedBuilder()
      .setTitle(`長文メッセージ違反 from ${message.guild.name}`)
      .setDescription(
        ...
        `**ポイント**: ${totalPoints}\n` +
        `**処罰**: ${punishment || 'なし'}\n` +
        `**文字数**: ${contentLength} (制限: ${settings.antiLongMessage.lengthLimit || 500})\n` +
        `**メッセージ**: ${message.content.slice(0, 1000)}`
      )
      .setTimestamp();
    try {
