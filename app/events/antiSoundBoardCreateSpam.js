...
export async function antiSoundboardCreateSpam(client) {
  client.on(Events.GuildAuditLogEntryCreate, async (auditLog) => {
    if (auditLog.action !== AuditLogEvent.SoundboardSoundCreate) return;
    ...
    settings.antiSoundboardCreateSpam = settings.antiSoundboardCreateSpam ?? { enabled: false, nukeLimit: 5, timeframe: 60000 };
    ...
    settings.ruleWhitelist.soundboard_create = settings.ruleWhitelist.soundboard_create ?? {
      channels: [], categories: [], roles: [], members: []
    };

    const isRuleWhitelisted =
      member.roles.cache.some((role) => settings.ruleWhitelist.soundboard_create.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.soundboard_create.members.some((m) => m === member.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiSoundboardCreateSpam?.enabled) return;

    const rateLimit = settings.antiSoundboardCreateSpam.nukeLimit || 5;
    const timeframe = settings.antiSoundboardCreateSpam.timeframe || 60000;
    const points = settings.points.soundboard_create || 4;
    const now = Date.now();
    const targetId = auditLog.targetId;

    if (!actionHistory.has(member.id)) {
      actionHistory.set(member.id, [{ timestamp: now, targetId }]);
    } else {
      const userActions = actionHistory.get(member.id);
      userActions.push({ timestamp: now, targetId });
      ...
      if (recentActions.length > rateLimit) {
        ...
        if (webhookUrl) {
          const embed = new EmbedBuilder()
            .setTitle(`サウンドボード作成制限違反 from ${auditLog.guild.name}`)
