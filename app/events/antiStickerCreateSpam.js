...
export async function antiStickerCreateSpam(client) {
  client.on(Events.GuildStickerCreate, async (sticker) => {
    const auditLogs = await sticker.guild.fetchAuditLogs({ type: AuditLogEvent.StickerCreate, limit: 1 }).catch(() => null);
    const entry = auditLogs?.entries.first();
    const userId = entry?.executorId || client.user.id;
    const member = await sticker.guild.members.fetch(userId).catch(() => null);
    if (!member) return;
    ...
    settings.antiStickerCreateSpam = settings.antiStickerCreateSpam ?? { enabled: false, nukeLimit: 5, timeframe: 60000 };
    ...
    settings.ruleWhitelist.sticker_create = settings.ruleWhitelist.sticker_create ?? {
      channels: [], categories: [], roles: [], members: []
    };

    const isRuleWhitelisted =
      member.roles.cache.some((role) => settings.ruleWhitelist.sticker_create.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.sticker_create.members.some((m) => m === member.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiStickerCreateSpam?.enabled) return;

    const rateLimit = settings.antiStickerCreateSpam.nukeLimit || 5;
    const timeframe = settings.antiStickerCreateSpam.timeframe || 60000;
    const points = settings.points.sticker_create || 3;
    const now = Date.now();

    if (!actionHistory.has(member.id)) {
      actionHistory.set(member.id, [{ timestamp: now, targetId: sticker.id }]);
    } else {
      const userActions = actionHistory.get(member.id);
      userActions.push({ timestamp: now, targetId: sticker.id });
      ...
      if (recentActions.length > rateLimit) {
        ...
        for (const action of recentActions) {
          const stickerToDelete = await sticker.guild.stickers.fetch(action.targetId).catch(() => null);
          if (stickerToDelete) await stickerToDelete.delete('ステッカー作成スパム検知').catch(() => {});
        }

        if (settings.block?.enabled && punishment) {
          try {
            if (punishment === 'timeout') await member.timeout(settings.block.timeout || 600000, 'ステッカー作成制限超過');
            else if (punishment === 'kick') await member.kick('ステッカー作成制限超過');
            else if (punishment === 'ban') await sticker.guild.members.ban(member.id, { reason: 'ステッカー作成制限超過' });
          } catch (err) {
        ...
        if (webhookUrl) {
          const embed = new EmbedBuilder()
            .setTitle(`ステッカー作成制限違反 from ${sticker.guild.name}`)
            .setDescription(`**サーバー**: ${sticker.guild.name} (${sticker.guild.id})\n**ユーザー**: ${member.user.tag} (${member.id})\n**ルール**: sticker_create\n**ポイント**: ${totalPoints}\n**処罰**: ${punishment || 'なし'}\n**アクション数**: ${recentActions.length}/${rateLimit}`)
            .setTimestamp();
          try {
