...
const actionHistory = new Map(); // ユーザーごとの削除履歴を保持
...
export async function handleWebhookDeleteSpam(client) {
  client.on(Events.WebhookUpdate, async (channel) => {
    ...
    settings.ruleWhitelist.webhook_delete = settings.ruleWhitelist.webhook_delete ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    const isRuleWhitelisted =
      settings.ruleWhitelist.webhook_delete.channels.some((ch) => ch === channel.id) ||
      settings.ruleWhitelist.webhook_delete.categories.some((cat) => cat === channel.parentId) ||
      member.roles.cache.some((role) => settings.ruleWhitelist.webhook_delete.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.webhook_delete.members.some((m) => m === member.id);

    settings.antiWebhookDeleteSpam = settings.antiWebhookDeleteSpam ?? { enabled: false, nukeLimit: 10, timeframe: 60000 };
    if (isWhitelisted || isRuleWhitelisted || !settings.antiWebhookDeleteSpam?.enabled) return;

    const rateLimit = settings.antiWebhookDeleteSpam.nukeLimit || 10;
    const timeframe = settings.antiWebhookDeleteSpam.timeframe || 60000;
    const points = settings.points.webhook_delete || 1;
    const now = Date.now();
    ...
    if (!actionHistory.has(member.id)) {
      ...
      if (recentActions.length > rateLimit) {
        ...
        if (settings.block.enabled && punishment) {
          try {
            if (punishment === 'timeout') {
              await member.timeout(settings.block.timeout || 600000, 'Webhook削除制限超過');
            } else if (punishment === 'kick') {
              await member.kick('Webhook削除制限超過');
            } else if (punishment === 'ban') {
              await channel.guild.members.ban(member.id, { reason: 'Webhook削除制限超過' });
            }
          } catch (err) {
        ...
        if (webhookUrl) {
          const embed = new EmbedBuilder()
            .setTitle(`Webhook削除制限違反 from ${channel.guild.name}`)
            .setDescription(
