...
export async function handleChannelCreateSpam(client) {
  client.on(Events.ChannelCreate, async (channel) => {
    ...
    settings.antiChannelCreateSpam = settings.antiChannelCreateSpam ?? { enabled: false, nukeLimit: 10, timeframe: 60000 };
    ...
    settings.ruleWhitelist.channel_create = settings.ruleWhitelist.channel_create ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    const isRuleWhitelisted =
      settings.ruleWhitelist.channel_create.channels.some((ch) => ch === channel.id) ||
      settings.ruleWhitelist.channel_create.categories.some((cat) => cat === channel.parentId) ||
      member.roles.cache.some((role) => settings.ruleWhitelist.channel_create.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.channel_create.members.some((m) => m === member.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiChannelCreateSpam?.enabled) return;

    const rateLimit = settings.antiChannelCreateSpam.nukeLimit || 10;
    const timeframe = settings.antiChannelCreateSpam.timeframe || 60000;
    const points = settings.points.channel_create || 1;
    const now = Date.now();
    ...
    if (!actionHistory.has(member.id)) {
      ...
      if (recentActions.length > rateLimit) {
        ...
        for (const action of recentActions) {
          const channelToDelete = await channel.guild.channels.fetch(action.targetId).catch(() => null);
          if (channelToDelete) await channelToDelete.delete().catch(() => {});
        }

        if (settings.block.enabled && punishment) {
          try {
            if (punishment === 'timeout') {
              await member.timeout(settings.block.timeout || 600000, 'チャンネル作成制限超過');
            } else if (punishment === 'kick') {
              await member.kick('チャンネル作成制限超過');
            } else if (punishment === 'ban') {
              await channel.guild.members.ban(member.id, { reason: 'チャンネル作成制限超過' });
            }
          } catch (err) {
        ...
        if (webhookUrl) {
          const embed = new EmbedBuilder()
            .setTitle(`チャンネル作成制限違反 from ${channel.guild.name}`)
            .setDescription(
