...
export async function handleAntiRaid(client) {
  client.on(Events.MessageCreate, async (message) => {
    ...
    settings.antiRaid = settings.antiRaid ?? { enabled: false, timeframe: 5000 };

    settings.ruleWhitelist = settings.ruleWhitelist ?? {};
    settings.ruleWhitelist.duplicate = settings.ruleWhitelist.raid ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    if (isWhitelisted || isRuleWhitelisted || !settings.antiRaid.enabled) return;

    const userMessages = messageHistory.get(message.author.id) || [];
    userMessages.push({ content: message.content, time: Date.now(), channelId: message.channelId });
    const timeframe = settings.antiRaid.timeframe || 5000;
    ...
    for (const channel of channels.values()) {
      ...
      for (const msg of messages.values()) {
        if (msg.author.id === message.author.id || msg.system) continue;
        if (Date.now() - msg.createdTimestamp > timeframe) continue;
        if (similarity(message.content, msg.content) >= 0.91) {
          violation = true;
          break;
        }
      }
    ...
    if (violation) {
      await handleViolation(message, 'raid', settings);
    }
  });
...
async function handleViolation(message, rule, settings) {
  ...
  if (webhookUrl) {
    const embed = new EmbedBuilder()
      .setTitle(`Anti-Raid Violation from ${message.guild.name}`)
      .setDescription(
