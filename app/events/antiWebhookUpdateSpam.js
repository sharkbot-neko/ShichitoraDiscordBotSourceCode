...
export async function handleWebhookUpdateSpam(client) {
  client.on(Events.WebhookUpdate, async (channel) => {
    ...
    settings.antiWebhookUpdateSpam = settings.antiWebhookUpdateSpam ?? { enabled: false, nukeLimit: 10, timeframe: 60000 };
    ...
    settings.ruleWhitelist.webhook_update = settings.ruleWhitelist.webhook_update ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    const isRuleWhitelisted =
      settings.ruleWhitelist.webhook_update.channels.some((ch) => ch === channel.id) ||
      settings.ruleWhitelist.webhook_update.categories.some((cat) => cat === channel.parentId) ||
      member.roles.cache.some((role) => settings.ruleWhitelist.webhook_update.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.webhook_update.members.some((m) => m === member.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiWebhookUpdateSpam?.enabled) return;

    const targetWebhookId = entry.target?.id;
    if (!targetWebhookId) return;

    const rateLimit = settings.antiWebhookUpdateSpam.nukeLimit || 10;
    const timeframe = settings.antiWebhookUpdateSpam.timeframe || 60000;
    const points = settings.points.webhook_update || 1;
    const now = Date.now();

    if (!actionHistory.has(member.id)) {
      actionHistory.set(member.id, [{ timestamp: now, targetId: targetWebhookId }]);
    } else {
      const userActions = actionHistory.get(member.id);
      userActions.push({ timestamp: now, targetId: targetWebhookId });
      ...
      if (recentActions.length > rateLimit) {
        ...
        try {
          ...
          pointsData = {};
          await fs.writeFile(pointsPath, JSON.stringify(pointsData, null, 2)).catch(console.error);
        }

        pointsData[guildId] = pointsData[guildId] || {};
        pointsData[guildId][member.id] = pointsData[guildId][member.id] || { points: 0, lastViolation: null };
        ...
        try {
          const webhook = await client.fetchWebhook(targetWebhookId);
          await webhook.delete('Webhook更新スパム対策').catch(() => {});
        } catch (err) {
        }
        ...
        if (settings.block.enabled && punishment) {
          try {
            if (punishment === 'timeout') {
              await member.timeout(settings.block.timeout || 600000, 'Webhook更新制限超過');
            } else if (punishment === 'kick') {
              await member.kick('Webhook更新制限超過');
            } else if (punishment === 'ban') {
              await channel.guild.members.ban(member.id, { reason: 'Webhook更新制限超過' });
            }
          } catch (err) {
        ...
        if (webhookUrl) {
          const embed = new EmbedBuilder()
            .setTitle(`Webhook更新制限違反 from ${channel.guild.name}`)
            .setDescription(
