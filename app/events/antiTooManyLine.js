...
export async function handleTooManyLine(client) {
  client.on(Events.MessageCreate, async (message) => {
    ...
    settings.antiTooManyLine = settings.antiTooManyLine ?? { enabled: false, lineLimit: 5, timeframe: 5000 };
    ...
    settings.ruleWhitelist.too_many_line = settings.ruleWhitelist.too_many_line ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    const isRuleWhitelisted =
      settings.ruleWhitelist.too_many_line.channels.some((ch) => ch === message.channelId) ||
      settings.ruleWhitelist.too_many_line.categories.some((cat) => cat === channel.parentId) ||
      message.member?.roles.cache.some((role) => settings.ruleWhitelist.too_many_line.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.too_many_line.members.some((m) => m === message.author.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiTooManyLine?.enabled) return;

    let lineCount = 0;

    lineCount += (message.content.match(/\n/g) || []).length + 1;

    if (message.messageSnapshots?.size) {
      try {
        const refMsg = await message.channel.messages.fetch(message.reference.messageId);
        lineCount += (refMsg.content.match(/\n/g) || []).length + 1;
      } catch (_) { }
    }
    ...
    if (lineCount >= (settings.antiTooManyLine.lineLimit || 5)) {
      await handleViolation(message, 'too_many_line', settings, lineCount);
    }
  });
}

async function handleViolation(message, rule, settings, lineCount) {
  ...
  if (webhookUrl) {
    const embed = new EmbedBuilder()
      .setTitle(`改行数超過違反 from ${message.guild.name}`)
      .setDescription(
