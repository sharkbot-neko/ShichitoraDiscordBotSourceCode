...
export async function handleCharacterRepeats(client) {
  client.on(Events.MessageCreate, async (message) => {
    ...
    settings.antiCharacterRepeats = settings.antiCharacterRepeats ?? { enabled: false, repeatLimit: 5, timeframe: 5000 };
    ...
    settings.ruleWhitelist.character_repeats = settings.ruleWhitelist.character_repeats ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    const isRuleWhitelisted =
      settings.ruleWhitelist.character_repeats.channels.some((ch) => ch === message.channelId) ||
      settings.ruleWhitelist.character_repeats.categories.some((cat) => cat === channel.parentId) ||
      message.member?.roles.cache.some((role) => settings.ruleWhitelist.character_repeats.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.character_repeats.members.some((m) => m === message.author.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiCharacterRepeats?.enabled) return;
    ...
    if (maxRepeats >= (settings.antiCharacterRepeats.repeatLimit || 5)) {
      await handleViolation(message, 'character_repeats', settings, maxRepeats);
    }
  });
}

async function handleViolation(message, rule, settings, maxRepeats) {
  ...
  if (webhookUrl) {
    const embed = new EmbedBuilder()
      .setTitle(`文字連続違反 from ${message.guild.name}`)
      .setDescription(
