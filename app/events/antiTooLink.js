...
export async function handleTooLink(client) {
  client.on(Events.MessageCreate, async (message) => {
    ...
    settings.antiTooLink = settings.antiTooLink ?? { enabled: false, linkLimit: 5 };
    ...
    settings.ruleWhitelist.too_link = settings.ruleWhitelist.too_link ?? { channels: [], categories: [], roles: [], members: [] };

    const isRuleWhitelisted =
      settings.ruleWhitelist.too_link.channels.some(ch => ch === message.channelId) ||
      settings.ruleWhitelist.too_link.categories.some(cat => cat === channel.parentId) ||
      message.member?.roles.cache.some(role => settings.ruleWhitelist.too_link.roles.some(r => r === role.id)) ||
      settings.ruleWhitelist.too_link.members.some(m => m === message.author.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiTooLink?.enabled) return;
    ...
    if (message.reference?.messageId) {
      try {
        ...
        totalLinks += (refMsg.content.match(urlRegex) || []).length;
      } catch { }
    }

    if (totalLinks >= (settings.antiTooLink.linkLimit || 5)) {
      await handleViolation(message, 'too_link', settings, totalLinks);
    }
  });
  ...
  async function handleViolation(message, rule, settings, count) {
    ...
    if (webhookUrl) {
      const embed = new EmbedBuilder()
        .setTitle(`リンク数超過違反 from ${message.guild.name}`)
        .setDescription(
