...
export async function antiAutoModRuleUpdateSpam(client) {
  client.on(Events.AutoModerationRuleUpdate, async (oldRule, newRule) => {
    const auditLogs = await newRule.guild.fetchAuditLogs({ type: AuditLogEvent.AutoModerationRuleUpdate, limit: 1 }).catch(() => null);
    const entry = auditLogs?.entries.first();
    const userId = entry?.executorId || client.user.id;
    const member = await newRule.guild.members.fetch(userId).catch(() => null);
    if (!member) return;
    ...
    settings.antiAutoModRuleUpdateSpam = settings.antiAutoModRuleUpdateSpam ?? { enabled: false, nukeLimit: 6, timeframe: 60000 };
    ...
    settings.ruleWhitelist.automod_update = settings.ruleWhitelist.automod_update ?? {
      channels: [], categories: [], roles: [], members: []
    };

    const isRuleWhitelisted =
      member.roles.cache.some((role) => settings.ruleWhitelist.automod_update.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.automod_update.members.some((m) => m === member.id);

    if (isWhitelisted || isRuleWhitelisted || !settings.antiAutoModRuleUpdateSpam?.enabled) return;

    const rateLimit = settings.antiAutoModRuleUpdateSpam.nukeLimit || 6;
    const timeframe = settings.antiAutoModRuleUpdateSpam.timeframe || 60000;
    const points = settings.points.automod_update || 5;
    const now = Date.now();

    if (!actionHistory.has(member.id)) {
      actionHistory.set(member.id, [{ timestamp: now, targetId: newRule.id }]);
    } else {
      const userActions = actionHistory.get(member.id);
      userActions.push({ timestamp: now, targetId: newRule.id });
      ...
      if (recentActions.length > rateLimit) {
        ...
        if (settings.block?.enabled && punishment) {
          try {
            if (punishment === 'timeout') await member.timeout(settings.block.timeout || 600000, 'AutoModルール更新制限超過');
            else if (punishment === 'kick') await member.kick('AutoModルール更新制限超過');
            else if (punishment === 'ban') await newRule.guild.members.ban(member.id, { reason: 'AutoModルール更新制限超過' });
          } catch (err) {
        ...
        if (webhookUrl) {
          const embed = new EmbedBuilder()
            .setTitle(`AutoModルール更新制限違反 from ${newRule.guild.name}`)
            .setDescription(`**サーバー**: ${newRule.guild.name} (${newRule.guild.id})\n**ユーザー**: ${member.user.tag} (${member.id})\n**ルール**: automod_update\n**ポイント**: ${totalPoints}\n**処罰**: ${punishment || 'なし'}\n**アクション数**: ${recentActions.length}/${rateLimit}`)
            .setTimestamp();
          try {
