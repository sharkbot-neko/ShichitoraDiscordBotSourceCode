...
export async function handleMarkdownSpam(client) {
  client.on(Events.MessageCreate, async (message) => {
    ...
    settings.ruleWhitelist.markdown_spam = settings.ruleWhitelist.markdown_spam ?? {
      channels: [],
      categories: [],
      roles: [],
      members: []
    ...
    const isRuleWhitelisted =
      settings.ruleWhitelist.markdown_spam.channels.some((ch) => ch === message.channelId) ||
      settings.ruleWhitelist.markdown_spam.categories.some((cat) => cat === channel.parentId) ||
      message.member?.roles.cache.some((role) => settings.ruleWhitelist.markdown_spam.roles.some((r) => r === role.id)) ||
      settings.ruleWhitelist.markdown_spam.members.some((m) => m === message.author.id);

    if (isWhitelisted || isRuleWhitelisted) return;

    settings.antiMarkdownSpam = settings.antiMarkdownSpam ?? { enabled: false, ratio: 0.3 };
    if (!settings.antiMarkdownSpam.enabled) return;

    const content = message.content;
    if (content.length === 0) return;

    const markdownChars = content.match(/([#*-_~>])/g) || [];
    const markdownCount = markdownChars.length;
    const totalLength = content.length;
    const ratio = markdownCount / totalLength;

    if (ratio >= (settings.antiMarkdownSpam.ratio || 0.3)) {
      await handleViolation(message, 'markdown_spam', settings, { ratio: ratio.toFixed(3), markdownCount, totalLength });
    }
  });
}

async function handleViolation(message, rule, settings, details) {
  const points = settings.points?.[rule] || 2;
  const guildId = message.guildId;
  ...
  if (settings.logWebhook) {
    const embed = new EmbedBuilder()
      .setTitle('Markdown装飾スパム違反')
      .setDescription(
        `**ユーザー**: ${message.author.tag} (${message.author.id})\n` +
        `**ルール**: ${rule}\n` +
        `**ポイント**: ${totalPoints}\n` +
        `**処罰**: ${punishment || 'なし'}\n` +
        `**装飾割合**: ${details.ratio * 100}%\n` +
        `**装飾文字数**: ${details.markdownCount}/${details.totalLength}\n` +
  ...
  if (webhookUrl) {
    const embed = new EmbedBuilder()
      .setTitle(`Markdown装飾スパム違反 from ${message.guild.name}`)
      .setDescription(
